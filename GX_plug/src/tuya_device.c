...
/*************************用户自定义函数**************************/
	dev_param.dev_dp.mot_stat.is_exist = TRUE;
	dev_param.dev_dp.mot_stat.dpid_num = 101;
	dev_param.mot_led.io_pin = TY_GPIOA_14;
	dev_param.mot_led.io_level = IO_LOW_LEVEL;
/****************************************************************/
...
VOID dev_obj_dp_cb(IN CONST TY_RECV_OBJ_DP_S *dp)
{
	...
	/**************************用户自定义函数*********************************/		
			case MOT_ST_TYPE:
                PR_DEBUG("DP101 arrived, value:%d",dp->dps[dp_index].value.dp_enum);
				tuya_hard_mot_control(dp->dps[dp_index].value.dp_value);
            break;
	/**************************************************************************/		
            default:
            break;
