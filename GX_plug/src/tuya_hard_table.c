...
typedef enum
{
    MOT_OFF,                               ///<relay is open on power
    MOT_ON,                              ///<relay is close on power
}MOT_POWER_STAT;
...
STATIC VOID tuya_hard_set_mot_stat(BOOL_T mot_stat);
...
VOID tuya_hard_init_dev_stat(VOID)
{
	...
	/*************************用户自定义函数**************************/
	g_dev_stat.mot_stat = MOT_OFF;
	PR_DEBUG("mot off already!");
...
VOID tuya_hard_init(DEVICE_PARAM_S *dev_info)
{
	...
	/*************************用户自定义函数**************************/
	g_dev_info.dev_dp.mot_stat.dpid_num = dev_info->dev_dp.mot_stat.dpid_num;
	g_dev_info.dev_dp.mot_stat.is_exist = dev_info->dev_dp.mot_stat.is_exist;
	
	g_dev_info.mot_led.io_config.io_pin = dev_info->mot_led.io_pin;
	g_dev_info.mot_led.io_config.io_level = dev_info->mot_led.io_level;
	op_ret = tuya_hard_led_init(&g_dev_info.mot_led);
    if (OPRT_OK != op_ret)
    {    	
        return;
    }
	PR_NOTICE("----------mot_led init ok------------");
...
OPERATE_RET tuya_hard_upload_all_data(VOID)
{
	...
	/****************************************用户自定义函数*********************************************/
	 if (g_dev_info.dev_dp.mot_stat.is_exist)
    {
        tuya_upload_dp_bool_frame(upload_dp,g_dev_info.dev_dp.mot_stat.dpid_num,g_dev_stat.mot_stat);
    }
	 /*******************************************************************************************************/
...
DEV_DP_TYPE_E tuya_hard_dpid_type(UINT_T app_send_dpid)
{
	...
	else if (app_send_dpid == g_dev_info.dev_dp.mot_stat.dpid_num)
    {
        return MOT_ST_TYPE;
    }
    else
...
/**
 * @Function: tuya_hard_set_channel_stat
 * @Description: 电机状态设置，包括电机状态和电机指示灯状态
 * @Input: channel_stat：通道状态
 * @Output: none
 * @Return: none
 * @Others: none
...
{
//    tuya_hard_set_mot
    tuya_hard_set_led(&g_dev_info.mot_led,LED_ON_OFF,(mot_stat == TRUE)?(1):(0));
...
/**
 * @Function: tuya_hard_channel_control
 * @Description: app下发状态对电机状态设置
 * @Input: app_send_switch_stat：app下发通道状态
 * @Output: none
 * @Return: none
 * @Others: none
 */
VOID tuya_hard_mot_control(BOOL_T app_send_mot_stat)
{
	g_dev_stat.mot_stat = app_send_mot_stat;
	
    tuya_hard_set_mot_stat(g_dev_stat.mot_stat);
}
